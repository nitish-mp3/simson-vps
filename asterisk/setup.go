...
exten => _X.,1,NoOp(Simson outbound trunk call ${EXTEN} via ${SIMSON_TRUNK})
 ...
 same  => n,GotoIf($["${SIMSON_POST_ANSWER_DTMF}" = ""]?dial-limit)
 same  => n,Set(SIMSON_DIAL_OPTIONS=${SIMSON_DIAL_OPTIONS}U(simson-outbound-postanswer^${SIMSON_POST_ANSWER_DTMF}))
 same  => n(dial-limit),GotoIf($["${SIMSON_MAX_CONNECTED_MS}" = ""]?dial)
 same  => n,Set(SIMSON_DIAL_OPTIONS=${SIMSON_DIAL_OPTIONS}L(${SIMSON_MAX_CONNECTED_MS}))
