...
func writePJSIPConf(root string, cfg SetupConfig, endpoints []SIPEndpointDef) error {
	...
	if cfg.ExternalIP != "" {
		sb.WriteString("external_media_address=" + cfg.ExternalIP + "\n")
		sb.WriteString("external_signaling_address=" + cfg.ExternalIP + "\n")
...
; ── ConfBridge profiles ───────────────────────────────────────────────────
; Profile name must match the second arg to ConfBridge() in the dialplan above.
; Asterisk recognises 'type=bridge' to distinguish bridge profiles from user profiles.
[simson_bridge]
type=bridge
max_members = 10
